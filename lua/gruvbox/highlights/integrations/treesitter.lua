...
function M.get(cp, config)
   cp.none = "NONE"
   local operators = cp.aqua
   local cl = cp.red -- conditionals, loops
   local keywords = cp.red
   local math_logic = cp.purple
   ...
   return {
      ...
      TSInclude = { fg = cp.aqua }, -- For includes: #include in C, use or extern crate in Rust, or require in Lua.
      TSOperator = { fg = operators }, -- For any operator: +, but also -> and * in cp.
      TSKeywordOperator = { fg = operators, style = "bold" }, -- For new keyword operator
      TSPunctSpecial = { fg = cp.orange, style = "bold" }, -- For special punctutation that does not fall in the catagories before.

      TSFloat = { fg = math_logic }, -- For floats.
      TSNumber = { fg = math_logic }, -- For all numbers
      TSBoolean = { fg = math_logic }, -- For booleans.
      ...
      TSException = { fg = cp.orange, style = config.styles.keywords }, -- For exception related keywords.
      ...
      TSNote = { fg = cp.bg2, bg = cp.blue },
      TSWarning = { fg = cp.bg2, bg = cp.yellow },
      TSDanger = { fg = cp.bg2, bg = cp.red },
      -- TSConstMacro        = { };    -- For constants that are defined by macros: NULL in cp.
      -- TSError = { fg = cp.red }, -- For syntax/parser errors.
      -- rustTSField = { fg = cp.bg4 }, -- For fields.
      TSLabel = { fg = cp.blue }, -- For labels: label: in C and :label: in Lua.
      TSMethod = { fg = cp.green }, -- For method calls and definitions.
      TSNamespace = { fg = cp.red }, -- For identifiers referring to modules and namespaces.
      -- TSNone              = { };    -- TODO: docs
      -- TSParameterReference= { };    -- For references to parameters of a function.
      tomlTSProperty = { fg = cp.blue }, -- Differentiates between string and properties
      TSPunctDelimiter = { fg = cp.orange }, -- For delimiters ie: .
      TSPunctBracket = { fg = cp.orange }, -- For brackets and parenthesis.
      TSString = { fg = cp.green }, -- For strings.
      TSStringRegex = { fg = cp.orange, style = config.styles.strings }, -- For regexes.
      -- TSSymbol            = { };    -- For identifiers referring to symbols or atoms.
      TSType = { fg = cp.yellow }, -- For types.
      TSVariable = { fg = cp.fg0, style = config.styles.variables }, -- Any variable name that does not have another highlighcp.
      TSTagAttribute = { fg = cp.purple }, -- Tags like html tag names.
      TSTag = { fg = cp.orange }, -- Tags like html tag names.
      TSTagDelimiter = { fg = cp.red_a }, -- Tag delimiter like < > /
      TSText = { fg = cp.fg0 }, -- For strings considerated text in a markup language.
      ...
      TSURI = { fg = cp.yellow, style = "italic,underline" }, -- urls, links and emails
      TSLiteral = { fg = cp.green }, -- used for inline code in markdown and for doc in python (""")
      TSTextReference = { fg = cp.aqua, style = "bold" }, -- references
      TSTitle = { fg = cp.blue, style = "bold" }, -- titles like: # Example
      TSEmphasis = { fg = cp.red_a, style = "italic" }, -- bold
      TSStrong = { fg = cp.red_a, style = "bold" }, -- italic
      TSStringEscape = { fg = cp.pink, style = config.styles.strings }, -- For escape characters within a string.
      ...
      -- lua
      luaTSField = { fg = cp.purple },
      luaTSConstructor = { fg = cp.flamingo }, -- For constructor calls and definitions: = { } in Lua, and Java constructors.

      -- java
      javaTSConstant = { fg = cp.aqua },

      -- typescript
      typescriptTSProperty = { fg = cp.blue }, -- Same as TSField.

      -- css
      cssTSType = { fg = cp.purple },
