...
function M.get(cp, config)
   local virtual_text = config.integrations.native_lsp.virtual_text
   local underlines = config.integrations.native_lsp.underlines
   ...
   return {
      ...
      -- documentation.
      LspReferenceText = { bg = cp.bg0 }, -- used for highlighting "text" references
      LspReferenceRead = { bg = cp.bg4 }, -- used for highlighting "read" references
      LspReferenceWrite = { bg = cp.bg4 }, -- used for highlighting "write" references
      ...
      LspCodeLens = { fg = cp.gray }, -- virtual text of the codelens
   }
end
